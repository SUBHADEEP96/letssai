...
const responseFromEvents = (
  createEvents: (signal: AbortSignal) => AsyncIterable<StreamEvent>,
      ...
      async start(controller) {
        try {
          ...
          if (!abortController.signal.aborted)
            controller.enqueue(
              encoder.encode(
                `${JSON.stringify({ type: "error", message: "The assistant is temporarily unavailable. Please try again or contact LetssAI." })}\n`
              )
            )
        } finally {
          controller.close()
...
async function* one(event: StreamEvent) {
  yield event
}
...
export async function POST(request: Request) {
  ...
  return responseFromEvents(
    (signal) => streamLetssAIAgent(parsed.data.messages, signal),
    request.signal
  )
}
