...
export class MemoryAnswerCache implements AnswerCache {
  ...
  constructor(
    private readonly maxEntries = Number(
      process.env.RAG_CACHE_MAX_ENTRIES || 200
    ),
    private readonly ttlMs = Number(process.env.RAG_CACHE_TTL_SECONDS || 900) *
      1000
  ) {}
  get(key: string) {
...
export const containsPII = (messages: ChatMessage[]) =>
  messages.some(({ content }) =>
    /\b(?:\+?\d[ ()-]?){7,16}\b|\b[A-Z0-9._%+-]+@(?!letssai\.com\b)[A-Z0-9.-]+\.[A-Z]{2,}\b/i.test(
      content
    )
  )
export function createAnswerCacheKey(
  messages: ChatMessage[],
  knowledgeVersion = process.env.RAG_KNOWLEDGE_VERSION ||
  ...
  const normalized = messages
    ...
    .map(({ role, content }) => ({
      role,
      content: content.trim().replace(/\s+/g, " ").toLowerCase().slice(0, 2000),
    }))
