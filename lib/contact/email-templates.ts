...
export function ownerEmail(input: ContactSubmissionInput) {
  const chatbotContext = input.chatbot
    ? `<p><strong>Chatbot context</strong><br>Challenge: ${escapeHtml(input.chatbot.identifiedBusinessChallenge)}<br>Recommended service: ${escapeHtml(input.chatbot.recommendedService || "Not yet identified")}<br>Current tools: ${escapeHtml(input.chatbot.currentTools || "Not supplied")}<br>Timeline: ${escapeHtml(input.chatbot.timeline || "Not supplied")}<br>Summary: ${escapeHtml(input.chatbot.conversationSummary)}</p>`
    : ""
  return shell(
